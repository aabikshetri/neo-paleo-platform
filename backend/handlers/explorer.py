...
from backend.repositories.runtime_data import (
...
from backend.runtime import df, taxa_df, dataset_publications_df, taxon_profiles_df
from backend.schemas.requests import (
    AnalogueRequest,
    CalibrationRequest,
    NmdsRequest,
    SampleProfilesRequest,
    TaxaAggregateRequest,
    TaxonValuesRequest,
)
...
def _selection_rows(token: str) -> list[dict]:
    try:
        filters = decode_selection(token)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    return search(**filters)


def _request_ids(request, attribute: str = "sampleids") -> list[int]:
    token = getattr(request, "selection_token", None)
    if token:
        return [int(row["sampleid"]) for row in _selection_rows(token)][:5000]
    return list(dict.fromkeys(getattr(request, attribute, [])))[:5000]
...
def modern_analogues(request: AnalogueRequest):
    candidate_ids = _request_ids(request, "calibration_sampleids")
...
def search_page(
    ...
    page_size: int = Query(250, ge=25, le=1000),
    ph_min: Optional[float] = None,
    ph_max: Optional[float] = None,
    water_min: Optional[float] = None,
    water_max: Optional[float] = None,
    ...
    filters = {
        ...
        "site_contains": site_contains,
        "publication_contains": publication_contains,
    }
    publicationid = None
    publication = publication_contains.strip() if publication_contains else ""
    if publication.startswith("publication:"):
        candidate = publication.removeprefix("publication:")
        if candidate.isdigit():
            publicationid = int(candidate)
    if using_postgres() and (not publication or publicationid is not None):
        rows, total, summary = search_page_postgres(
            page=page,
            page_size=page_size,
            publicationid=publicationid,
            **{key: value for key, value in filters.items() if key != "publication_contains"},
        )
    else:
        summary_rows = search(**filters)
        total = len(summary_rows)
        start = (page - 1) * page_size
        rows = summary_rows[start:start + page_size]
        ph_values = [float(row["pH"]) for row in summary_rows if row.get("pH") is not None]
        water_values = [
            float(row["water_table_depth"])
            for row in summary_rows if row.get("water_table_depth") is not None
        ]
        summary = {
            "samples": total,
            "sites": len({row.get("siteid") for row in summary_rows}),
            "mean_pH": sum(ph_values) / len(ph_values) if ph_values else None,
    ...
    return {
        ...
        "page": page,
        "page_size": page_size,
        "selection_token": encode_selection(filters),
        "summary": summary,
    }


def selection_rows(selection_token: str = Query(..., min_length=1, max_length=4096)):
    return _selection_rows(selection_token)
